import { IsInt, IsNotEmpty, IsOptional, Max, Min, IsUUID } from 'class-validator';
import { ApiProperty } from '@nestjs/swagger';

export class CreateReviewDto {
  @ApiProperty({ description: 'Puntuación del espacio de trabajo (1-5)', example: 4 })
  @IsInt({ message: 'La puntuación del espacio de trabajo debe ser un número entero' })
  @Min(1, { message: 'La puntuación mínima permitida es 1' })
  @Max(5, { message: 'La puntuación máxima permitida es 5' })
  @IsNotEmpty({ message: 'La puntuación del espacio de trabajo es obligatoria' })
  coworking_rating: number;

  @ApiProperty({ description: 'Comentario opcional', example: 'Excelente ambiente y ubicación' })
  @IsOptional()
  comment?: string;
