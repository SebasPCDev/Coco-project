export const FilterCoworks = (coworks: any[], filter: any) => {
  ...
  const funcionfiltrar = () => {
    return toFilter.filter((element) => {
      ...
      if (
        filterElement.location.country &&
        filterElement.location.country !== element.country
      ) {
        return false;
      }
      if (
        filterElement.location.state &&
        filterElement.location.state !== element.state
      ) {
        return false;
      }
      if (
        filterElement.location.city &&
        filterElement.location.city !== element.city
      ) {
        return false;
      }
      ...
      if (filterElement.capacity.length > 0) {
        ...
        if (
          !filterElement.capacity.some((range) => {
