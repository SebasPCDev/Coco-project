...
export class CreateBookingsDto {
  ...
  @IsString({ message: 'El tiempo de reserva debe ser una cadena de caracteres.' })
  @IsNotEmpty({ message: 'El tiempo de reserva es obligatorio.' })
  @IsTimeRange({ message: 'El tiempo de reserva debe estar en un rango válido.' })
  reservationTime: Date;

  @IsEmpty({ message: 'La fecha de creación no debe estar presente en la solicitud.' })
  date: Date;

  @IsEmpty({ message: 'El estado de la reserva no debe estar presente en la solicitud.' })
  ...
  @ApiProperty({ type: String, example: '7d78f279-7e3c-4b32-bf7e-df4e14571e2b' })
...
export class UpdateBookingsDto {
  ...
  @IsEnum(BookingStatus, { message: 'El estado de la reserva debe ser uno de los valores permitidos: PENDING, COMPLETED, CANCELLED.' })
  status: BookingStatus
